...
- **2026-06-10 — reprise + auto-arrêt superviseur** : constat = relance utilisateur incomplète (EXP down depuis le 06/06 ~19:54Z ; MAIN up via python *système* puis mort ; boucles OFF ; exporter arrêté depuis le 06/06 19:18Z ; monitor arrêté le 08/06 20:37Z après ~23 h de « RAS, DOWN »). **0 trade depuis le reset** (tables `paper_trade` vides) — pendant la seule fenêtre vivante (06/06 18:10→19:54Z, feed live), le gate v2.1.0 a correctement rejeté les signaux sous-rentables. Relance depuis la session CLI : EXP+MAIN via `launch_backend.py` (venv), mode `experimental` + boucles 60 s ON, exporter ON — ⚠️ processus **liés à la session CLI** (meurent si elle ferme). Feed **LIVE** confirmé (BTC ~$61.9k, source binance, timestamp temps réel). Ajout **règle d'auto-arrêt** au superviseur (`claude-monitor-prompt.md`, commit `5fb8800` repo MAIN) : 2 backends down > 1 h → dernière entrée horodatée puis fin de boucle/session. **Action utilisateur (durabilité)** : BTC kill previous → BTC start-all (IntelliJ) + BTC monitor.
